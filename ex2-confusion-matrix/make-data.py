...
# Add a Klingon Clone of the Warpshuttle that has the same statistics as the Romulan Warpshuttle
    ...
    for k in range(0, 1000):
        ...
        for j in range(0, TOTAL_STATS):
            if ('Clone' in ships_labels[i]):
                ...
                rand_stat.append(
                    round(uniform(stats[i-1][j][0], stats[i-1][j][1]), 2))
            else:
                rand_stat.append(
                    round(uniform(stats[i][j][0], stats[i][j][1]), 2))
        print(', '.join(str(item)
